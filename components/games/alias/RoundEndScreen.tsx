...
export function RoundEndScreen({ room, playerId }: Props) {
  ...
  return (
    <div className="relative flex flex-col items-center flex-1 px-5 py-10 h-screen-safe overflow-y-auto no-scrollbar">
      <motion.div
        variants={stagger}
        initial="hidden"
        animate="show"
        className="relative w-full max-w-[400px] flex flex-col gap-5"
      >
        {/* Header */}
        <motion.div variants={fadeUp} className="text-center">
          <p className="text-[10px] text-white/30 tracking-[0.25em] uppercase mb-2">Runda {room.round} završena</p>
          <h2
            className="text-[28px] font-extrabold tracking-[-0.3px]"
            style={{ color: colors.accent2, textShadow: `0 0 20px ${hexA(colors.accent, 0.5)}` }}
          >
            {colors.label}
          </h2>
        ...
        <motion.div variants={fadeUp} className="flex gap-2 w-full">
          {[
            ...
            >
              ...
              <p className="text-[8px] uppercase tracking-[0.15em] mt-0.5 text-white/30">{s.l}</p>
            </div>
          ))}
        </motion.div>
        ...
        <motion.div variants={fadeUp}>
          <p className="text-[9px] text-white/30 tracking-[0.25em] uppercase mb-3">Riječi</p>
          <div
            className="flex flex-col rounded-2xl overflow-hidden border border-white/10 bg-white/[0.05]"
          ...
          >
            {room.roundResults.map((r, i) => (
              <motion.div
                ...
                style={{ borderBottom: i < room.roundResults.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}
              >
                <span className="text-[13px] text-white/70">{r.word}</span>
                <span
          ...
          {isHost ? (
            <button
              ...
              style={{
                ...
                border: `1px solid ${hexA(colors.accent, 0.5)}`,
                boxShadow: `0 4px 20px ${hexA(colors.accent, 0.3)}`,
              }}
            >
              Rezultati
            </button>
