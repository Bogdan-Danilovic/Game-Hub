...
import { createRoom, joinRoom } from '@/lib/firestore/flip7';
...
export function HomeScreen() {
  ...
  const nameValid =
    trimmedName.length >= 2 && trimmedName.length <= 16 && NAME_REGEX.test(trimmedName);
  ...
  async function handleCreate() {
    ...
    try {
      ...
      router.push(`/games/flip7/room/${code}`);
    } catch (err) {
    ...
    <div className="relative flex flex-col items-center justify-center flex-1 px-8 h-screen-safe overflow-hidden">
      <div className="relative w-full max-w-[320px] flex flex-col gap-12">
        <motion.div {...fadeIn(0.2)} className="flex flex-col items-start gap-6">
          <motion.div
            className="text-4xl"
            animate={{ rotate: [0, -4, 4, -2, 0], y: [0, -3, 0] }}
          ...
          <div>
            <h1 className="text-[48px] font-bold tracking-[-0.04em] leading-[0.9] text-amber-50">
              Flip 7
            </h1>
            <p className="mt-3 text-[13px] text-amber-100/50 leading-relaxed max-w-[260px]">
              Vuci karte i znaj kada da staneš — pre nego što izvučeš duplikat.
            </p>
          </div>
        </motion.div>
        ...
        <motion.div {...fadeIn(0.6)} className="flex flex-col gap-4">
          ...
          <div className="flex gap-3 items-end">
            ...
            <Button
              ...
              onClick={handleJoin}
              className="shrink-0 mb-[1px] !border-amber-500/40 !text-amber-400 hover:!border-amber-400 hover:!text-amber-300"
            >
              {loading === 'join' ? '...' : 'Uđi'}
            </Button>
          </div>
