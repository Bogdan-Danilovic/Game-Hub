...
import { createRoom, joinRoom } from '@/lib/firestore/uno';
import { getGameById } from '@/lib/games/registry';
import { hexA } from '@/lib/utils';
import { useAuth } from '@/hooks/useAuth';

const GAME = getGameById('uno')!;
const ACCENT = GAME.accentColor;
const ONLINE = '#30d158';
const RULES = [
  'Baci kartu iste boje, broja ili tipa kao na vrhu.',
  'Ako nemaš odgovarajuću — vuci iz špila.',
  'Wild karta uvijek prolazi — biraš boju.',
  'Ostala ti jedna karta? Vikni "UNO!" odmah!',
  'Ko prvi ostane bez karata — osvaja rundu!',
];
...
export function HomeScreen() {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      router.push(`/games/uno/room/${code}`);
    } catch (err) {
          ...
          <div className="flex items-end gap-3">
            <div className="flex-1">
              <Input
                ...
                maxLength={5}
                className="text-center text-[16px] font-bold uppercase tracking-[0.4em]"
              />
            </div>
            <Button
              variant="secondary"
              ...
              onClick={handleJoin}
              className="mb-[1px] shrink-0 !rounded-2xl"
            >
              {loading === 'join' ? '...' : 'Uđi'}
            </Button>
          </div>
