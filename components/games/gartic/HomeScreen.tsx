...
import { createRoom, joinRoom } from '@/lib/firestore/gartic';
import { useAuth } from '@/hooks/useAuth';

const GAME = getGameById('gartic')!;
const ACCENT = GAME.accentColor;
const ONLINE = '#30d158';
const RULES = [
  'Svako napiše rečenicu — to je start tvoje knjige.',
  'Sljedeći igrač crta tvoju rečenicu bez da je vidi.',
  'Treći opisuje crtež tekstom — i tako dalje u krug.',
  'Na kraju se otkriva koliko je originalna poruka mutirala!',
];
...
export function HomeScreen() {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      router.push(`/games/gartic/room/${code}`);
    ...
    <div className="flex-1 overflow-y-auto no-scrollbar">
      <div className="mx-auto flex w-full max-w-[400px] flex-col gap-6 px-5 pb-14 pt-20">
        ...
        <motion.div {...fadeIn(0.3)} className="flex flex-col gap-4">
          <Input label="Tvoje ime" placeholder="Unesi ime" value={name}
            onChange={(e) => setName(e.target.value)} maxLength={16} autoComplete="off"
            className="!rounded-2xl !border-white/10 !bg-white/[0.05] !text-white focus:!border-pink-500/60 focus:!ring-pink-500/25" />
          ...
          <div className="flex items-end gap-3">
            <div className="flex-1">
              <Input label="Kod sobe" placeholder="_ _ _ _ _" value={roomCode}
                onChange={(e) => setRoomCode(e.target.value.toUpperCase())} maxLength={5}
                className="!rounded-2xl !border-white/10 !bg-white/[0.05] !text-white text-center text-[16px] font-bold uppercase tracking-[0.4em] focus:!border-pink-500/60 focus:!ring-pink-500/25" />
            </div>
