        ...
        <div className="w-full max-w-[320px] flex flex-col gap-8">
          ...
          >
            <div className="p-6">
              <div className="flex items-start justify-between mb-5">
                ...
                <div className="flex flex-wrap gap-1.5 justify-end">
                  {GAME?.tags?.slice(0, 2).map((tag: string) => (
                    <span
                      key={tag}
                      className="text-[9px] px-2 py-0.5 rounded-full border border-white/10 text-white/40 uppercase tracking-wider"
                    >
                      {tag}
                    </span>
                  ))}
              ...
              <h1 className="text-[32px] font-bold tracking-[-0.04em] text-white leading-none mb-2">
                {GAME?.name ?? 'Šta bi nacrtao?'}
              </h1>
              <p className="text-[12px] text-white/40 leading-relaxed mb-5">
                Jedan crta, ostali pogađaju — ko prvi pogodi osvaja bodove.
              </p>

              <div className="flex items-center gap-4 text-[11px] text-white/30">
                <span className="flex items-center gap-1.5">
                  <Users size={12} />
                  {GAME?.minPlayers ?? 3}–{GAME?.maxPlayers ?? 8} igrača
                </span>
                <span className="flex items-center gap-1.5">
                  <Clock size={12} />
                  {GAME?.avgDuration ?? '15-25 min'}
                </span>
              </div>
            </div>
          </motion.div>
            ...
            <div className="flex gap-3 items-end">
              <div className="flex-1">
                <Input
                  ...
                  maxLength={5}
                  className="!rounded-2xl !border-white/10 !bg-white/[0.05] !text-white text-center tracking-[0.4em] uppercase font-bold text-[16px] focus:!border-amber-500/60"
                />
              </div>
              <Button
                variant="secondary"
          ...
          {error && (
            <motion.p
              ...
              className="text-[13px] text-amber-400/90 -mt-4"
            >
              {error}
            </motion.p>
          )}
